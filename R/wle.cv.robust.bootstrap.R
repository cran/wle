...
wle.cv.rb <- function(formula, data=list(), model=TRUE, x=FALSE, y=FALSE, contrasts=NULL, monte.carlo=500, split=NULL, R=1000, sim=c('betaR', 'uniformR', 'fastRBwc'), weights=NULL, min.weight=0.5, num.max=NULL, nmodel=NULL, file.name=NULL, append=FALSE,  mean=0.4, shape=NULL, nsize=NULL, boot.resid=FALSE, boot.param=FALSE, replace=TRUE, control=wle.lm.control(), ...) {
...
#####      zparam=mat.or.vec(monte.carlo,nvar+nzero),
      info=integer(1),
      PACKAGE="wle")
  ...
  if (length(wcv0)) {
    var.in <- posizione*wcv0[posizione]
    var.in <- var.in[-var.in!=0]
    t0.param <- rep(0, nvar)
    temp.lm <- summary(lm(ydata~xdata[,var.in] -1, weights=dpesi))
    sigma <- dpesi%*%(temp.lm$residuals^2)/(sum(dpesi) - length(var.in))
    t0.param[var.in] <- c(temp.lm$coeff[,1])
    t0.param <- c(t0.param, sigma, wcv0[nvar+1])
...
#  fine nmodel
    ...
    if (length(wcv)) {      
      ...
      sigma <- dpesiboot%*%(temp.lm$residuals^2)/(sum(dpesiboot) - length(var.in))
      param[var.in] <- temp.lm$coeff[,1]
      param <- c(param, sigma, wcv[nvar+1])

...
wle.normal.high.old <- function(x, boot=30, group, num.sol=1, raf="HD", smooth, tol=10^(-6), equal=10^(-3), max.iter=500, use.smooth=TRUE, tol.int, verbose=FALSE) {
...
if (equal<=tol) {
  ...
  z <- .Fortran("wlenormhigh",
	...
	mahalanobis=mat.or.vec(num.sol,size),                
...
if (z$nsol>1) {
    ...
    result$mahalanobis <- z$mahalanobis[1:z$nsol,]

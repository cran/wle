      SUBROUTINE WLECP (YDATA,XDATA,INTER,NSIZE,NVAR,
...
C     Weighted Mallows Cp based on 
...
C     ALPHA     input    D      1            the coefficients of the number of the parameters.
C
C
...
C         write(*,*) nmaxsol*(imodel-1)+isol,i,nmaxsol
 ...
 80      continue
     ...
     &        wsum/dvar - (dtotpesi*dsize) + (alpha*ncol) 
C      write(*,*) cp(nmaxsol*(imodel-1)+isol,npre+1)
 ...
 85      continue
         ...
         cp(nmaxsol*(imodel-1)+isol,npre+1)=
     &   wsum/dvar - (wtotpesi(isol)*dsize) + (alpha*ncol)  
C      write(*,*) cp(nmaxsol*(imodel-1)+isol,npre+1)
      endif
